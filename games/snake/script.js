...
class Table {
    ...
    display() {
        ...
        this.notes.forEach(row => {
            code += "<tr>"
            row.forEach(elem => {
                code += `<td class="tron_row" id="${elem.row}${elem.col}" style="background-color:${elem.fill == 0 ? Fills.Def : elem.fill != -1 ? Fills.Blue : Fills.Red};">`
                code += "</td>"
            })
            code += "</tr>"
        })
...
function start() {
    ...
    get("allapot").style.color = Fills.Def;
    get("allapot").innerHTML = ""
...
function stopper(player) {
    stop++;
    get("allapot").style.color = player.fill;
    get("allapot").innerHTML = player.name + " has died..."
